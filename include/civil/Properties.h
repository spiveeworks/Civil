...
public:
	...
	PropertyFormat* child (index line_index, byte child_id) const
    {
    ...
    family_type::size_type child_id(family_lines_type::const_iterator line, PropertyFormat *child_format) const;

    family_type::size_type child_id(index line_index, PropertyFormat *child_format) const
    {
...
struct Property
{
	...
	typedef std::map<PropertyFormat*, std::vector<byte> > data_type;  // usage: byte datum = data[&branch_format][datum_index];
	data_type data;
...
template<class in_it_t>
in_it_t Property::read_in(in_it_t in_it, PropertyFormat* branch)
{
    check_branch(branch);
...
template<class out_it_t>
out_it_t Property::write_out(out_it_t out_it, PropertyFormat* branch) const
{
